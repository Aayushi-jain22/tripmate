...
```
agent/
  state.py         State schema — {"messages": [...]} with LangGraph's add_messages reducer
  graph.py          The StateGraph itself: agent node, tools node, conditional routing
  llm_factory.py    Picks ChatOllama or ChatAnthropic based on TRIPMATE_PROVIDER
  orchestrator.py   Thin run-loop: invokes the compiled graph, rebuilds AgentResponse/trace
  tool_schemas.py   SYSTEM_PROMPT only (tool JSON schemas are now auto-generated by @tool)
  config.py         Environment-variable settings (unchanged)
  logging_setup.py  Structured JSON logging (unchanged)
tools/
  rag_tool.py           Pure retrieval logic (unchanged) — TF-IDF search over destination_data.json
  weather_tool.py       Pure lookup logic (unchanged) — mock seasonal weather table
  langchain_tools.py    @tool wrappers around the two files above; TOOLS list the graph binds
tests/
  test_rag_tool.py, test_weather_tool.py   Unchanged — unit tests for the pure tool logic
  test_tool_selection.py                   Graph routing/trace tests, LLM decisions scripted
  test_integration.py                      End-to-end: scripted LLM + real tools through the graph
```

---

## What changed from the custom implementation

| | Custom (previous submission) | LangGraph (this submission) |
|---|---|---|
| **Control flow** | Hand-rolled `for turn in range(max_turns)` loop in `orchestrator.py`, manually appending messages and checking `stop_reason` | Declarative `StateGraph` with `agent` / `tools` nodes and a conditional edge (`agent/graph.py`) |
| **Tool dispatch** | A `dict[str, Callable]` (`_TOOL_DISPATCH`) manually mapping tool names to functions | `ToolNode(TOOLS)`, LangGraph's prebuilt dispatcher — new tools only need to be added to a list |
| **Tool schemas** | Hand-written JSON schemas in `agent/tool_schemas.py`, kept in sync with the tool functions by hand | Auto-generated by `@tool` from each wrapper's type hints + docstring (`tools/langchain_tools.py`) |
| **Provider abstraction** | A custom `NormalizedMessage`/`TextBlock`/`ToolUseBlock` shape (`agent/llm_types.py`) + a hand-written Anthropic↔Ollama adapter (`agent/ollama_client.py`) | Standard LangChain `BaseChatModel` interface — `ChatAnthropic` / `ChatOllama` are drop-in, no adapter code to maintain |
| **State** | Implicit — a growing `list[dict]` of raw message dicts threaded through the loop | Explicit typed `AgentState` (`agent/state.py`), merged via LangGraph's `add_messages` reducer |
| **Turn-cap safety valve** | Manual `max_turns` loop counter | LangGraph's built-in `recursion_limit`, raising `GraphRecursionError` |
| **Underlying tool logic** | `tools/rag_tool.py`, `tools/weather_tool.py` | **Unchanged** — same files, same functions, same unit tests |
| **Trace / logging format** | JSON events from inside the orchestrator's `_execute_tool` | Same JSON events, now emitted from the `@tool` wrappers themselves |

The retrieval and weather logic was deliberately left untouched: the
assessment's second stage asks for a better *agentic workflow* around
existing skills, not a rewrite of the skills themselves. Every unit test
in `test_rag_tool.py` / `test_weather_tool.py` still passes unmodified.
